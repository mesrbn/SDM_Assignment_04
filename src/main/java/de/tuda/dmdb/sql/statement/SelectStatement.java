...
import de.tuda.dmdb.sql.operator.exercise.EquiJoin;
import de.tuda.dmdb.sql.operator.exercise.Projection;
import de.tuda.dmdb.sql.operator.exercise.Selection;
	...
	private Error analyze() {
		...
		if (this.wherePredicate != null) {
			for (SimplePredicate simplePred : this.wherePredicate.getTerms()) {
				...
				EnumSQLType type1 = tab1.getType(tab1.getColumnNumber(att1
						.getName()));
				EnumSQLType type2 = null;
